...
- If a single hard-linked or reflinked/cow'd file is added to RAM(instead of an entire directory), either hard link or reflink reference will be lost upon doing backups using '--mmbackup', '--mtbackup', '--mmremove' and only after it has been edited/changed/updated in RAM. If the file hasn't been edited/changed/updated while in RAM, doing backups shouldn't be a problem since no data differences will be being found and nothing will be backed up to DISK in this case.

**About Current Limitations:** Aside from reflinks/CoW'd files not being supported on TMPFS/RAMFS; As for hard links being located outside directory tree structure being added to RAM, currently there's no way for retrieving information on multiple directories and filenames through it's inode number alone without actually searching & looking for the inode in the entire file system, this however would be impractical because it would require a very large ammount of time the larger the filesystem is.

**Workaround:** If you really need to work with hard linked and reflinked/cow'd files, you can still write your own backup tool(s) for it, creating the hard-links again and reflinking wherever necessary.
